// pages/departswift/departswift.ts
Page({
  ...
  inputHead: function(e) {
    this.setData({
      transferDepartmentId: e.detail.value, // 更新部门名称
    });
  },
  // 用户输入备注时触发
  ...
  handleSwift: function (e) {
    ...
    console.log(this.data.localDateTime);
          wx.request({
            url: 'http://localhost:8080/employee/InsertDepartment', // 修改为你的后端接口地址
            method: 'PUT',
            data: {
              employeeId: this.data.employee.employeeId,//员工号
              transferDepartmentId: this.data.transferDepartmentId,//部门号

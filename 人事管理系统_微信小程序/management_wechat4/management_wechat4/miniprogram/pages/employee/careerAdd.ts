Page({
  ...
  onLoad: function (options) {
    ...
    inputOrganizationName: function (e) {
      const value = e.detail.value;
      this.setData({
        'employee.organizationName': value, // 更新部门名称
      });
    },
      // 用户输入部门名称时触发
    ...
    const value = e.detail.value;
    this.setData({
      'employee.position': value, // 更新部门名称
    });
  },
  // 用户输入部门名称时触发
  inputJobDescription: function (e) {
    const value = e.detail.value;
    this.setData({
      'employee.jobDescription': value, // 更新部门名称
    });
  },
  // 用户输入部门名称时触发
  ...
  savePosition: function () {
    ...
    const newCareer = {
      ...
      startTime: this.data.employee.startTime,
      endTime: this.data.employee.endTime,
      organizationName:this.data.employee.organizationName,
      position: this.data.employee.position,
      jobDescription: this.data.employee.jobDescription,
      salary: this.data.employee.salary,
      note: this.data.employee.note
      // 其他部门信息字段
    };
    employee.career.push(newCareer);
    wx.request({
        ...
        success: (res) => {
          if (res.confirm) {
            // 用户点击了确定按钮，向后端发送删除请求
              ...
              this.setData({
                'employee.relation': career
              });
              
            wx.navigateTo({
              url: '/pages/employee/careerAdd?employee=' + JSON.stringify(employee),
            });
          }
        }
      });
